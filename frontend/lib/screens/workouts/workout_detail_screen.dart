...
class _WorkoutDetailScreenState extends ConsumerState<WorkoutDetailScreen> {
  ...
  List<Widget> _buildAccessorySetsForLift(List<WorkoutSet> accessorySets) {
    // Group accessory sets by exercise
    final groupedSets = <String, List<WorkoutSet>>{};
    for (final set in accessorySets) {
      final key = set.exerciseId ?? 'exercise_${accessorySets.indexOf(set) ~/ (accessorySets.first.prescribedReps ?? 5)}';
      groupedSets.putIfAbsent(key, () => []).add(set);
    }

    // Separate standalone and circuit exercises
    final standaloneExercises = <String, List<WorkoutSet>>{};
    final circuitExercises = <int, Map<String, List<WorkoutSet>>>{};
    ...
    for (final entry in groupedSets.entries) {
      ...
      if (circuitGroup == null) {
        ...
        circuitExercises.putIfAbsent(circuitGroup, () => {});
        circuitExercises[circuitGroup]![exerciseId] = sets;
      }
    }

    final widgets = <Widget>[];

    // Build standalone exercise widgets
    for (final entry in standaloneExercises.entries) {
      widgets.add(_buildAccessoryExerciseWidget(entry.key, entry.value));
    }

    // Build circuit widgets
    final sortedCircuits = circuitExercises.keys.toList()..sort();
    for (final circuitNum in sortedCircuits) {
      widgets.add(_buildCircuitWidget(circuitNum, circuitExercises[circuitNum]!));
    }

    return widgets;
  }

  Widget _buildAccessoryExerciseWidget(String exerciseId, List<WorkoutSet> sets) {
    final exercise = _exercises[exerciseId];
    final exerciseName = exercise?.name ?? 'Accessory Exercise';
    final isCompleted = _workoutDetail!.status == 'COMPLETED';
    final displayText = isCompleted
        ? '${sets.length} sets: ${sets.map((s) => "${s.actualReps ?? 0} @ ${s.actualWeight?.toInt() ?? 0} lbs").join(", ")}'
        : '${sets.length} × ${sets.first.prescribedReps ?? 0} reps';
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    'Target: ${sets.length} × ${sets.first.prescribedReps ?? 0} reps',
                    style: TextStyle(
  ...
  Widget _buildCircuitWidget(int circuitNum, Map<String, List<WorkoutSet>> exercises) {
    final isCompleted = _workoutDetail!.status == 'COMPLETED';
      ...
      child: Column(
        ...
        children: [
          ...
          Container(
            ...
            child: Row(
              children: [
                Icon(Icons.repeat, size: 14, color: Colors.orange.shade800),
                const SizedBox(width: 6),
                Text(
                  'Circuit $circuitNum',
                  style: TextStyle(
            ...
            child: Column(
              children: exercises.entries.map((entry) {
                ...
                return Padding(
                  ...
                  child: Row(
                    children: [
                      Icon(Icons.fitness_center, size: 12, color: Colors.orange.shade700),
                      const SizedBox(width: 6),
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
  ...
  Widget _buildAccessoryExercisesCard(List<WorkoutSet> accessorySets) {
        ...
        child: Column(
          ...
          children: [
            ...
            ...groupedSets.entries.map((entry) {
              ...
              final exerciseName = firstSet.exerciseId != null && _exercises.containsKey(firstSet.exerciseId)
                  ? _exercises[firstSet.exerciseId]!.name
                  : 'Accessory Exercise';
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 4),
                          Text(
                            '${sets.length} sets × ${firstSet.prescribedReps ?? 0} reps',
                            style: Theme.of(context).textTheme.bodyMedium?.copyWith(
                                  color: Colors.grey.shade700,
                                ),
                          ),
  ...
  String _getDisplayMainLift(String lift) {
    switch (lift) {
      case 'press':
